...
#### Wait N seconds to combine user messages

<img width="893" height="711" alt="image" src="https://github.com/user-attachments/assets/dde610cb-8b4b-49fc-956a-c5324ed15ccd" />

You can specify the interval during which the chatbot doesn't answer. Instead, it combines them into one and then replies altogether when the user stops sending new messages (and after the interval has passed). It may be useful to make the chatbot more natural, but sometimes answering every single message is preferable. Set the value to 0 (zero) to stop waiting.
...
#### The Crisp label
Switch this setting on to hide the label "bot" by Crisp.

![image](https://github.com/user-attachments/assets/e271ae9e-1f9c-4580-97e4-2dbb4b138a44)
...
![image](https://github.com/user-attachments/assets/55c73a9c-53eb-46bd-87df-ad8debb120cd)

**Never translate predefined messages automatically (like greeting and other predefined messages)**

By default, all the messages you enter, are translated automatically into the language the question was asked in. If you set this settings on, the messages will be sent as is, without translation.
...
Why would you use this feature? It can enrich your users' experience and collect more valuable information for you and your users. For example, you can :
...
So, what exactly happens when you set this setting on? We read the value of a clicked button, and form the text query like "User clicked the button 'sorry'", so the chatbot can react according to the prompts and settings.
...
![image](https://github.com/user-attachments/assets/4127467d-b44b-4109-bb82-7283b7351353)
...
![image](https://github.com/user-attachments/assets/f926ad62-5627-4603-860c-125d84ceea7e)
...
### Custom filters
These settings allow you to set up filters based on specific users' data, such as contact or custom data. 

<img width="1407" height="630" alt="image" src="https://github.com/user-attachments/assets/0dc14710-6744-4c9b-81df-b7378a720600" />

#### Conditions clause
Select "ANY" or "ALL" from the dropdown ("any" is the default value). If you select "any" it means that the chatbot will ignore the conversation as soon as **any** of conditions in the list are true, and if you select "all" it means that the chatbot will check all the conditions and will pause only when all of the conditions are met.

In the list of filters, you can add as many conditions as you want. The first value is the field name. Here is the typical conversation data:
...
```
If you are familiar with the JSON format, using it is not a problem. If you are not familiar, please check [this reference](https://www.freecodecamp.org/news/what-is-json-a-json-file-example/). The idea is we can access any data using the "dot notation". In this example, to get the user_name we will use ```data.user_name``` and to access a city, we will use ```data.geolocation.city```.

The second column is the condition, it has 4 values:
...
These values are self-explanatory. For example, if you would like to filter those chats belonging to a specific segment, you would use ```segment``` for a data name and ```contains``` as a condition.

The third column has a value to compare. You can add any value you would like to compare with: it may be numbers, strings, or even objects and lists. 

<Callout>
  Never use quotes for plain strings. Only use quotas if you specify fields for an object, or elements in the list.
</Callout>
